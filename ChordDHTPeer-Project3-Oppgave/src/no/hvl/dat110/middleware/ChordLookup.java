...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		NodeInterface highest_pred = null;
		...
		if (Util.computeLogic(node.getNodeID(), key, successor.getNodeID())) {
			return successor;
			
		// if logic returns false; call findHighestPredecessor(key)
		} else if (!Util.computeLogic(node.getNodeID(), key, successor.getNodeID())){
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for (int i = 0; i<fingertable.size(); i++) {
			...
			if (node.getNodeID().compareTo(stub.getNodeID()) <= 0 && 
					(stub.getNodeID().compareTo(key) <= 0)){
